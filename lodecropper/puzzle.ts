...
constructor(canvas:HTMLCanvasElement[],imagen:HTMLImageElement){
	...
	//variables para  cronomecro
	 ...
	 console.log(' PUZZLE ')   ;
	 this.imagenes=[];
	 //da formato a las imagenes en Htmlimagen y lo inserta en un array
		 canvas.forEach(element=>{ 
			 // conviero a HtmlImage cada corte y los valorres predeterminados
 ...
 /// numero de cortes dados
	var tam:integer=this.getBaseLog(2,this.imagenes.length);
	/**
	 
	var tam_width=this.rompecabezas.clientWidth/n +100;
	var tam_higth= this.rompecabezas.clientHeight/n +100;
	this.cuerpo.style.setProperty("--ancho",tam_width.toString()+"px")
	this.cuerpo.style.setProperty("--alto",tam_higth.toString()+"px")
	this.cuerpo.style.setProperty("--columnas",(n).toString())

	console.log("----------------------Imagen -----------------------------")
	console.log(this.cuerpo.style.getPropertyValue("--ancho") )
	console.log(this.cuerpo.style.getPropertyValue("--alto") )
	console.log(this.cuerpo.style.getPropertyValue("--columnas") )

	var tam={
	width:screen.width/n -10,
	heigth:screen.height/n -10
}
	 */
